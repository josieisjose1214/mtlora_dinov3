...
class WinDecoderTransformer(nn.Module):
    ...
    def forward(self, src, pos_embed, mask, pqs, **kwargs):
        ...
        qH, qW = query_feats.shape[-2:]
        # memory 与 query 的 window_partition 必须产生相同的 num_windows*B，才能过 MultiheadAttention。
        # - encode_src 与 PQ 网格同分辨率（常见 sparse）：memory 与 query 用相同 dec_win。
        # - dense 的 PQ 在 2× 分辨率上采样（如 224 vs 112）：须用 dec_win/2 划分 memory，
        #   使 (H/(Dh/2))*(W/(Dw/2)) = (2H/Dh)*(2W/Dw) 与 query 侧一致（与原版 PET 一致）。
        if qH == h and qW == w:
            mh, mw = self.dec_win_h, self.dec_win_w
        else:
            mh, mw = max(1, self.dec_win_h // 2), max(1, self.dec_win_w // 2)
        memory_win, pos_embed_win, mask_win = enc_win_partition(
            src, pos_embed, mask, mh, mw
        )
