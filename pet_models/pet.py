...
class BasePETCount(nn.Module):
    ...
    def points_queris_embed(self, samples, stride=8, src=None, **kwargs):
        ...
        pq_grid = kwargs.get('pq_grid_hw', None)
        if pq_grid is not None:
            # 与 context_encoder 输出/特征图同分辨率，保证 decoder 里 memory 与 query 窗口数一致
            gh = gw = int(pq_grid)
            ih, iw = int(input.shape[2]), int(input.shape[3])
            dev = input.device
            shift_y_1d = ((torch.arange(gh, device=dev, dtype=torch.float32) + 0.5) * (float(ih) / gh))
            shift_x_1d = ((torch.arange(gw, device=dev, dtype=torch.float32) + 0.5) * (float(iw) / gw))
            shift_y, shift_x = torch.meshgrid(shift_y_1d, shift_x_1d, indexing='ij')
            points_queries = torch.stack(
                [shift_y.flatten(), shift_x.flatten()], dim=1
            ).long()
            points_queries[:, 0].clamp_(0, ih - 1)
            points_queries[:, 1].clamp_(0, iw - 1)
            h, w = gh, gw
        else:
            ...
            h, w = int(shift_x.shape[0]), int(shift_x.shape[1])
        ...
        # 特征图空间尺寸可与 stride 网格不一致（例如上采样到更高分辨率），按图像坐标比例映射到特征格点
        _, _, H, W = src.shape
        img_h, img_w = float(input.shape[2]), float(input.shape[3])
        py = points_queries[:, 0].float().to(src.device)
        px = points_queries[:, 1].float().to(src.device)
        shift_y_down = (py * H / img_h).long().clamp(0, H - 1)
        shift_x_down = (px * W / img_w).long().clamp(0, W - 1)
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses):
        ...
        # pq_stride 可能为 8/4/2（取决于 PET 头里 sparse/dense 的 stride 配置）
        self.div_thrs_dict = {8: 0.0, 4: 0.5, 2: 0.5}
    ...
    def loss_labels(self, outputs, targets, indices, num_points, log=True, **kwargs):
        ...
        if 'div' in kwargs:
            ...
            den = torch.tensor([target['density'] for target in targets], device=src_logits.device)
            ...
            empty_w = self.empty_weight.to(src_logits.device)
            weights = target_classes.clone().float()
            weights[weights==0] = empty_w[0]
            weights[weights==1] = empty_w[1]
            raw_ce_loss = F.cross_entropy(src_logits.transpose(1, 2), target_classes, ignore_index=-1, reduction='none')

            # binarize split map
            split_map = kwargs['div']
            div_thrs = self.div_thrs_dict.get(int(outputs['pq_stride']), 0.5)
            ...
            loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight.to(src_logits.device), ignore_index=-1)
    ...
    def loss_points(self, outputs, targets, indices, num_points, **kwargs):
        ...
        if 'div' in kwargs:
            ...
            den = torch.tensor([target['density'] for target in targets], device=idx[0].device)
            den_sort = torch.sort(den)[1]
            n = len(den_sort)
            half = n // 2
            img_ds_idx = den_sort[:half]
            img_sp_idx = den_sort[half:]
            # batch_size==1 时 half==0，img_ds_idx 为空，torch.cat([]) 会报错；DDP 每卡 batch 常为 1
            parts_ds = [torch.where(idx[0] == bs_id)[0] for bs_id in img_ds_idx]
            parts_sp = [torch.where(idx[0] == bs_id)[0] for bs_id in img_sp_idx]
            pt_ds_idx = torch.cat(parts_ds) if len(parts_ds) > 0 else torch.zeros(0, dtype=torch.long, device=idx[0].device)
            pt_sp_idx = torch.cat(parts_sp) if len(parts_sp) > 0 else torch.zeros(0, dtype=torch.long, device=idx[0].device)
