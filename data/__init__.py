from .build import build_loader as _build_loader, build_nyud, build_pascal, build_wheat
...
def build_loader(config, simmim=False, is_pretrain=False, val_only=False):
    ...
    if config.get('DATA',{}).get('Wheat', False):
        return build_wheat(config, val_only)
    if not simmim:
