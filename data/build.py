...
def build_mtl(config, db_name="NYUD", val_only=False):
    ...
    multi_task_train_dataset, train_datasets = get_mtl_train_dataset(
        db_name, config, train_transforms)
    multi_task_val_dataset,val_datasets = get_mtl_val_dataset(db_name, config, val_transforms)
    data_loader_train = get_mtl_train_dataloader(config, multi_task_train_dataset, train_datasets)
    data_loader_val = get_mtl_val_dataloader(config, multi_task_val_dataset, val_datasets)
    mixup_fn = None

    return multi_task_train_dataset, multi_task_val_dataset, data_loader_train, data_loader_val, mixup_fn
...
def build_wheat(config,val_only):
    return build_mtl(config, 'Wheat', val_only=val_only)