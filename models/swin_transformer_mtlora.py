...
class Mlp(nn.Module):
    ...
    def forward(self, x, x_tasks=None, current_task=None):
        x, fc1_lora_tasks = self.fc1(x, x_tasks, current_task)
        x = self.act(x)
        x = self.drop(x)
        if fc1_lora_tasks is not None:
            tasks_to_process = [current_task] if current_task is not None and current_task in self.tasks else (self.tasks if self.tasks is not None else [])
            for task in tasks_to_process:
                if task in fc1_lora_tasks:
                    fc1_lora_tasks[task] = self.act(fc1_lora_tasks[task])
                    fc1_lora_tasks[task] = self.drop(fc1_lora_tasks[task])
        x, fc2_lora_tasks = self.fc2(x, fc1_lora_tasks, current_task)
        x = self.drop(x)
        if fc2_lora_tasks is not None:
            tasks_to_process = [current_task] if current_task is not None and current_task in self.tasks else (self.tasks if self.tasks is not None else [])
            for task in tasks_to_process:
                if task in fc2_lora_tasks:
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, current_task=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
            current_task: Current task name for task-specific LoRA computation
        """
        B_, N, C = x.shape
        qkv, _ = self.qkv(x, None, current_task)
        ...
        x, x_proj_lora_tasks = self.proj(x, None, current_task)
        x = self.proj_drop(x)
        if x_proj_lora_tasks is not None:
            # 仅对本次返回的任务做 drop（传入 current_task 时只含当前任务）
            for task in x_proj_lora_tasks:
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, current_task=None):
        ...
        attn_windows, attn_windows_lora_tasks = self.attn(
            x_windows, mask=self.attn_mask, current_task=current_task)
        ...
        if attn_windows_lora_tasks is not None:
            ...
            tasks_to_process = [current_task] if current_task is not None and current_task in self.tasks else (self.tasks if self.tasks is not None else [])
            for task in tasks_to_process:
                if task in attn_windows_lora_tasks:
        ...
        mlp_x_tasks = None
        if attn_windows_lora_tasks is not None:
            if current_task is not None and current_task in self.tasks and current_task in attn_windows_lora_tasks:
                mlp_x_tasks = {current_task: self.norm2(attn_windows_lora_tasks[current_task])}
            elif self.tasks is not None:
                mlp_x_tasks = {task: self.norm2(attn_windows_lora_tasks[task]) for task in self.tasks if task in attn_windows_lora_tasks}
        
        mlp_result, mlp_lora_tasks = self.mlp(
            self.norm2(x), mlp_x_tasks, current_task=current_task)
        ...
        if mlp_lora_tasks is None:
            ...
            if attn_windows_lora_tasks is None:
                tasks_to_process = [current_task] if current_task is not None and current_task in self.tasks else (self.tasks if self.tasks is not None else [])
                for task in tasks_to_process:
                    if task in mlp_lora_tasks:
                        mlp_lora_tasks[task] = self.drop_path(mlp_lora_tasks[task])
            else:
                tasks_to_process = [current_task] if current_task is not None and current_task in self.tasks else (self.tasks if self.tasks is not None else [])
                for task in tasks_to_process:
                    if task in mlp_lora_tasks and task in attn_windows_lora_tasks:
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, current_task=None):
        ...
        x = self.norm(x)
        # CompatLinear只接受2个参数，MTLoRALinear接受3个参数
        if isinstance(self.reduction, CompatLinear):
            x, _ = self.reduction(x, None)
        else:
            x, _ = self.reduction(x, None, current_task)
    ...
    def forward(self, x, current_task=None):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                # checkpoint不支持current_task参数，需要特殊处理
                x = checkpoint.checkpoint(blk, x)
                tasks_lora = None
            else:
                x, tasks_lora = blk(x, current_task=current_task)
                if tasks_lora is not None:
                    # 确保当前任务（如果指定）在tasks_lora中
                    if current_task is not None and current_task in self.tasks:
                        if current_task not in tasks_lora:
                            tasks_lora[current_task] = x
                    elif self.tasks is not None:
                        # 如果没有指定current_task，确保所有任务都在tasks_lora中
                        for task in self.tasks:
                            if task not in tasks_lora:
                                tasks_lora[task] = x
        if self.downsample is not None:
            # PatchMerging可能使用CompatLinear或MTLoRALinear，需要检查类型
            if hasattr(self.downsample, 'reduction') and isinstance(self.downsample.reduction, CompatLinear):
                # CompatLinear不支持current_task参数
                x, _ = self.downsample(x)
                if tasks_lora is not None:
                    tasks_to_process = [current_task] if current_task is not None and current_task in self.tasks else (self.tasks if self.tasks is not None else [])
                    for task in tasks_to_process:
                        if task in tasks_lora:
                            tasks_lora[task], _ = self.downsample(tasks_lora[task])
            else:
                # MTLoRALinear支持current_task参数
                x, _ = self.downsample(x, current_task=current_task)
                if tasks_lora is not None:
                    tasks_to_process = [current_task] if current_task is not None and current_task in self.tasks else (self.tasks if self.tasks is not None else [])
                    for task in tasks_to_process:
                        if task in tasks_lora:
                            tasks_lora[task], _ = self.downsample(tasks_lora[task], current_task=current_task)
        return x, tasks_lora
...
class SwinTransformerMTLoRA(nn.Module):
    ...
    def forward_features(self, x, return_stages=False, flatten_ft=False, current_task=None):
        ...
        for layer in self.layers:
            x, tasks_lora = layer(x, current_task=current_task)
            if tasks_lora is None:
                # 如果没有任务特定的LoRA输出，为当前任务（如果指定）或所有任务创建
                if current_task is not None and current_task in self.tasks:
                    tasks_lora = {current_task: x}
                elif self.tasks is not None:
    ...
    def forward(self, x, return_stages=False, flatten_ft=False, current_task=None):
        x = self.forward_features(x, return_stages, flatten_ft, current_task=current_task)
        x = self.head(x)
        return x
