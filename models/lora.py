...
class MTLoRALinear(LoRALayer):
    ...
    def forward(self, x: torch.Tensor, x_tasks: Dict[str, torch.Tensor] = None, current_task: str = None):
        """
        Args:
            x: Input tensor
            x_tasks: Task-specific input tensors (dict)
            current_task: Current task name. If provided, only compute LoRA for this task.
        """
        # TODO: handle merging
        pretrained = self.linear(x)
        if self.r == 0:
        ...
        # 如果指定了current_task，只计算当前任务的LoRA
        if current_task is not None and self.tasks is not None:
            if current_task not in self.tasks:
                # 如果当前任务不在任务列表中，只返回共享LoRA
                lora = (x @ self.lora_shared_A.transpose(0, 1)
                        @ self.lora_shared_B.transpose(0, 1)) * self.lora_shared_scale
                return pretrained + lora, {current_task: pretrained + lora}
            ...
            # 只计算当前任务的LoRA
            task_input = x if x_tasks is None or current_task not in x_tasks else x_tasks[current_task]
            if self.shared_mode == 'matrix':
                task_lora = pretrained + (task_input @ self.lora_tasks_A[current_task].transpose(
                    0, 1) @ self.lora_tasks_B[current_task].transpose(0, 1) * self.lora_task_scale[current_task])
            elif self.shared_mode == 'matrixv2':
                task_lora = pretrained + lora + (task_input @ self.lora_tasks_A[current_task].transpose(
                    0, 1) @ self.lora_tasks_B[current_task].transpose(0, 1) * self.lora_task_scale[current_task])
            elif self.shared_mode == 'addition':
                task_lora = pretrained + (task_input @ self.lora_tasks_A[current_task].transpose(
                    0, 1) @ self.lora_tasks_B[current_task].transpose(0, 1) * self.lora_task_scale[current_task])
            else:
                task_lora = pretrained + lora
            
            return pretrained + lora, {current_task: task_lora}
        
        # 原始逻辑：计算所有任务的LoRA（用于兼容性）
        if self.shared_mode == 'matrix':
            lora = (x @ self.lora_shared_A.transpose(0, 1)
                    @ self.lora_shared_B.transpose(0, 1)) * self.lora_shared_scale
            lora_tasks = {
...
class MTLoRAQKV(LoRALayer):
    ...
    def forward(self, x: torch.Tensor, x_tasks: Dict[str, torch.Tensor] = None, current_task: str = None):
        q_out, q_tasks = self.q(x, x_tasks, current_task)
        k_out, k_tasks = self.k(x, x_tasks, current_task)
        v_out, v_tasks = self.v(x, x_tasks, current_task)
        
        qkv_out = torch.cat([q_out, k_out, v_out], dim=-1)
        
        if current_task is not None and self.tasks is not None and current_task in self.tasks:
            # 只返回当前任务的输出
            qkv_tasks = {current_task: torch.cat([
                q_tasks[current_task] if q_tasks is not None and current_task in q_tasks else q_out,
                k_tasks[current_task] if k_tasks is not None and current_task in k_tasks else k_out,
                v_tasks[current_task] if v_tasks is not None and current_task in v_tasks else v_out
            ], dim=-1)}
        elif self.tasks is not None:
            # 返回所有任务的输出（兼容模式）
            qkv_tasks = {task: torch.cat([
                q_tasks[task] if q_tasks is not None and task in q_tasks else q_out,
                k_tasks[task] if k_tasks is not None and task in k_tasks else k_out,
                v_tasks[task] if v_tasks is not None and task in v_tasks else v_out
            ], dim=-1) for task in self.tasks} if (q_tasks is not None or k_tasks is not None or v_tasks is not None) else None
        else:
            qkv_tasks = None
            
        return qkv_out, qkv_tasks
