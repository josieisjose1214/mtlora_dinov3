...
def _to_plain_python(obj):
    """Recursively convert EasyDict/dict-like objects to plain Python containers."""
    if isinstance(obj, dict):
        return {k: _to_plain_python(v) for k, v in obj.items()}
    if isinstance(obj, tuple):
        return tuple(_to_plain_python(v) for v in obj)
...
_C.DATA.MIX_OPT=0
_C.DATA.RATIO=0
_C.DATA.BIN_ON=False
_C.DATA.BIN_SIZE=64
_C.DATA.BIN_GROW_RATIO=0.5
...
# DINOv3 backbone + DPT decoder parameters
_C.MODEL.DINOV3 = CN()
_C.MODEL.DINOV3.MODEL_NAME = 'convnext_small'  # convnext_small / vit_small / vit_base / vit_large
_C.MODEL.DINOV3.PRETRAINED = ''
_C.MODEL.DINOV3.PATCH_SIZE = 16
_C.MODEL.DINOV3.HIDDEN_DIM = 256
_C.MODEL.DINOV3.FPN_STRIDES = [4, 8, 16, 32]
...
_C.MODEL.DECODER_HEAD['classify'] = 'hrnet'
_C.MODEL.DECODER_HEAD['detect'] = 'hrnet'
_C.MODEL.DECODER_HEAD['count'] = 'pet'
_C.MODEL.DECODER_CHANNELS = [18, 36, 72, 144]
# PET 计数头：融合后特征再上采样到该空间尺寸；需满足 RES_SPARSE % 16 == 0（quadtree AvgPool 核 8x16）
# 推荐折中 96/192（相对 112/224 约省 ~27% 特征像素）；要更快试 80/160，要最高精度试 112/224
_C.MODEL.PET_COUNT_RES_SPARSE = 96
_C.MODEL.PET_COUNT_RES_DENSE = 192
...
def update_config(config, args):
    ...
    elif _check_args('wheat'):
        config.DATA.Wheat = args.wheat
        config.DATA.DATA_PATH = args.wheat
        db_name = "Wheat"
    ...
    if _check_args('tasks'):
        ...
        tsk_config = _to_plain_python(tsk_config)
    ...
    if hasattr(args, 'local_rank'):
        ...
        config.LOCAL_RANK = getattr(args, 'local-rank', 0)
    else:
        config.LOCAL_RANK = 0
    ...
    if config.MODEL.MTLORA.ENABLED:
        if config.MODEL.TYPE == 'swin':
            num_lora_layers = len(config.MODEL.SWIN.DEPTHS)
        else:
            num_lora_layers = 4
        if not isinstance(config.MODEL.MTLORA.R, list):
            config.MODEL.MTLORA.R = [
                config.MODEL.MTLORA.R] * num_lora_layers
        elif len(config.MODEL.MTLORA.R) == 1:
            config.MODEL.MTLORA.R = config.MODEL.MTLORA.R * \
                num_lora_layers
        else:
            assert len(config.MODEL.MTLORA.R) == num_lora_layers, "MTLoRA ranks length should be the same as the number of layers"
        if not isinstance(config.MODEL.MTLORA.SHARED_SCALE, list):
            config.MODEL.MTLORA.SHARED_SCALE = [
                config.MODEL.MTLORA.SHARED_SCALE] * num_lora_layers
        elif len(config.MODEL.MTLORA.SHARED_SCALE) == 1:
            config.MODEL.MTLORA.SHARED_SCALE = config.MODEL.MTLORA.SHARED_SCALE * \
                num_lora_layers
        else:
            assert len(config.MODEL.MTLORA.SHARED_SCALE) == num_lora_layers, "MTLoRA shared scale length should be the same as the number of layers"
        if not isinstance(config.MODEL.MTLORA.TASK_SCALE, list):
            config.MODEL.MTLORA.TASK_SCALE = [
                config.MODEL.MTLORA.TASK_SCALE] * num_lora_layers
        elif len(config.MODEL.MTLORA.TASK_SCALE) == 1:
            config.MODEL.MTLORA.TASK_SCALE = config.MODEL.MTLORA.TASK_SCALE * \
                num_lora_layers
        else:
            assert len(config.MODEL.MTLORA.TASK_SCALE) == num_lora_layers, "MTLoRA task scale length should be the same as the number of layers"
        if not isinstance(config.MODEL.MTLORA.DROPOUT, list):
            config.MODEL.MTLORA.DROPOUT = [
                config.MODEL.MTLORA.DROPOUT] * num_lora_layers
        elif len(config.MODEL.MTLORA.DROPOUT) == 1:
            config.MODEL.MTLORA.DROPOUT = config.MODEL.MTLORA.DROPOUT * \
                num_lora_layers
        else:
            assert len(config.MODEL.MTLORA.DROPOUT) == num_lora_layers, "MTLoRA dropout length should be the same as the number of layers"
            ...
            for task in config.TASKS + ['shared']:
                if not isinstance(config.MODEL.MTLORA.R_PER_TASK[task], list):
                    config.MODEL.MTLORA.R_PER_TASK[task] = [
                        config.MODEL.MTLORA.R_PER_TASK[task]] * num_lora_layers
                elif len(config.MODEL.MTLORA.R_PER_TASK[task]) == 1:
                    config.MODEL.MTLORA.R_PER_TASK[task] = config.MODEL.MTLORA.R_PER_TASK[task] * \
                        num_lora_layers
                else:
                    assert len(config.MODEL.MTLORA.R_PER_TASK[task]) == num_lora_layers, "MTLoRA ranks length should be the same as the number of layers"
            ...
            for task in config.TASKS:
                if not isinstance(config.MODEL.MTLORA.SCALE_PER_TASK[task], list):
                    config.MODEL.MTLORA.SCALE_PER_TASK[task] = [
                        config.MODEL.MTLORA.SCALE_PER_TASK[task]] * num_lora_layers
                    ...
                    assert len(config.MODEL.MTLORA.SCALE_PER_TASK[task]) == num_lora_layers, "MTLoRA task scale length should be the same as the number of layers"
